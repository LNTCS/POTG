...
import kotlinx.android.synthetic.main.activity_tactical_visor.*
import kr.edcan.potg.view.StickerView
import java.io.File
import java.io.FileOutputStream
import java.util.*

class TTVisorActivity : AppCompatActivity() {
    val PICK_IMAGE = 1234
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_tactical_visor)
        title = resources.getString(R.string.act_visor)
        ...
        btnSave.setOnClickListener {
            ...
            try {
                ...
                file = File(folder, resources.getString(R.string.act_visor) + "${System.currentTimeMillis() % 1000}.png")
                while(file!!.exists()){
                    file = File(folder, resources.getString(R.string.act_visor) + "${System.currentTimeMillis() % 1000}.png")
                }
                if (!file.exists()) {
        ...
        stickerTarget.setOnClickListener {
            addStickerView(R.drawable.target);
        }
    }

    private fun addStickerView(res: Int) {
        val stickerView = StickerView(this, 0.4f)
        stickerView.setImageResource(res)
        stickerView.setScale(0.09f, 1.2f)
